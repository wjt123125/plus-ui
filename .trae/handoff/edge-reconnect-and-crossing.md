...
- **议题二（补丁档已完成，2026-09-16 用户浏览器验证通过）**：dagre 分层照旧，跑完后对每个 gateway 的直接扇出分支层做后处理修正——按 branchIndex/outlet 顺序重新分配分支节点 x 坐标（嵌套子树 BFS 整体平移，~80 行），weight hack 与 merge constraint:false 已删。根治档自研布局三轮失败被否决（第二轮改崩线断连）；elkjs 评估后不换，未来出现补丁档覆盖不到的交叉再升级。下文议题二章节保留为决策存档。
